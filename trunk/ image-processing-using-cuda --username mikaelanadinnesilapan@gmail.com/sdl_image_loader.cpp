...
extern "C" void loadImageSDLFloat ( float4 **dst, int *w, int *h, const char *file ) {
	...
	*dst = (float4 *)malloc(sizeof(float4)*image->w*image->h);
	*w = image->w;
	*h = image->h;
	...
	if (notBMP){		
		...
		for(int x=0; x<*w; x++){
			for(int y=0; y<*h; y++){
				pixel = getpixel(image, x, y);		
				(*dst)[(*w)*y+x].x = (float)((pixel) & 0xff)/255.f;
				(*dst)[(*w)*y+x].y = (float)((pixel>>8) & 0xff)/255.f;
				(*dst)[(*w)*y+x].z = (float)((pixel>>16) & 0xff)/255.f;
				(*dst)[(*w)*y+x].w = (float)((pixel>>24) & 0xff)/255.f;
			}
		}
	}else{ //change order of colors. GRB in bmp images. :)
		//copy from surface to unsigned int destination array
